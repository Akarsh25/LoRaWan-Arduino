...
float _frequencies[] =
{
    902.3,902.5,902.7,902.9, 903.7, 903.3, 903.9
};

// How long should a frequency be monitored
#define FREQ_TIME_MS            25000
...
void rf95_setFrequency(uint32_t index)
{
    ...
    snprintf(_printBuffer, sizeof(_printBuffer), "Freq: %04u,Source: %d,MsgID: %d  ", (uint32_t)(_frequencies[index] * 10),rf95.headerFrom(),rf95.headerId());
...
void loop()
{
    ...
    if (rf95.recv(_rxBuffer, &_rxRecvLen))
    {
        ...
        //uint16_t _snr = spiRead(RH_RF95_REG_19_PKT_SNR_VALUE);
        //uint16_t _cr = spRH_RF95_CODING_RATE;
        snprintf(_printBuffer, sizeof(_printBuffer), "%u,%u.%u,%04d,%d,%d,%d,%d,%02X,%s",
            millis(),
